...
namespace Modules\Core\Http\Controllers\Api\V1\Permissions;
...
class PermissionController extends BaseController implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using('core::permission-list'), only: ['index']),
            new Middleware(PermissionMiddleware::using('core::permission-store'), only: ['store']),
            new Middleware(PermissionMiddleware::using('core::permission-show'), only: ['show']),
            new Middleware(PermissionMiddleware::using('core::permission-update'), only: ['update']),
            new Middleware(PermissionMiddleware::using('core::permission-delete'), only: ['destroy']),
        ];
    }
